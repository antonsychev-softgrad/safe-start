...
class AbstractPdfPlugin extends AbstractPlugin
{
  ...
  protected function drawVehicleHeader($vehicleData = array(), $companyData = array(), $additionalVehicleInfo = array())
  {
    if ($this->checkList) {
...
//      $data['Vehicle Make'] = $vehicleData['title'];
//    }
//    if($vehicleData['type']){
//        $data['Vehicle Model'] = $vehicleData['type'];
//    }
    ...
    if ($additionalVehicleInfo) {
      foreach($additionalVehicleInfo as $additionalInfo){
        $data[$additionalInfo->title] = $additionalInfo->default_value;
      }
    }
    ...
    if (isset($vehicleData['expiryDate']) && !empty($vehicleData['expiryDate'])) $data['Registration Expiry'] = date($this->getController()->moduleConfig['params']['date_format'], $vehicleData['expiryDate']);
