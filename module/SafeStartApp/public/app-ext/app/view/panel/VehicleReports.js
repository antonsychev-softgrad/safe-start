Ext.define('SafeStartExt.view.panel.VehicleReports', {
    ...
    updateDataView: function() {
        ...
        SafeStartExt.Ajax.request({
            url: 'vehicle/' + this.vehicle.get('id') + '/statistic', 
            data: post, 
            success: function(result) {
                if (result.statistic) {
                    ...
                    self.down('#SafeStartVehicleReportContent1').update(data);
                    if (result.statistic.chart && result.statistic.chart.length) {
                        self.down('#SafeStartVehicleReportChart').show();
                        self.down('#SafeStartVehicleReportChart').getAxes()[1].setDateFormat(SafeStartExt.dateFormat);
                        self.down('#SafeStartVehicleReportChart').getAxes()[1].setFromDate(self.down('datefield[name=from]').getValue());
                        self.down('#SafeStartVehicleReportChart').getAxes()[1].setToDate(self.down('datefield[name=to]').getValue());
                        var store = self.down('#SafeStartVehicleReportChart').getStore();
                        store.removeAll();
